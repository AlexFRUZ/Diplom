...
Інтелектуальна система розпізнавання **віку**, **статі** та **емоцій** людини за зображенням обличчя з графічним інтерфейсом користувача. Реалізована на основі трьох незалежно натренованих моделей **Vision Transformer (ViT)**. Розроблена в рамках кваліфікаційної роботи бакалавра.

## ✨ Функціональні можливості

- 🖥️ Сучасний графічний інтерфейс на базі **PyQt6**.
...
- ⚙️ Гнучке керування параметрами у реальному часі: мінімальний розмір обличчя, максимальна кількість облич у кадрі, ширина оброблюваного кадру.
...
> Файли ваг (~340 МБ кожен) не зберігаються у репозиторії — їх треба натренувати локально через ноутбук `Face_ViT_SingleTask_and_YOLO.ipynb` або запитати у автора.
...
Натренуй власні, прогнавши `Face_ViT_SingleTask_and_YOLO.ipynb` (потрібен Kaggle API ключ — `~/.kaggle/kaggle.json`), або помісти готові `.pt`-файли у корінь проєкту:
...
Проєкт розроблено в академічних цілях. Датасети (UTKFace, FER) належать їхнім авторам і використовуються відповідно до умов на Kaggle.

## 👤 Автор

**Олександр Гладун** — кваліфікаційна робота бакалавра, 2026.
