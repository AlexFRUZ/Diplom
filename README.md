...
**S**ociocultural **C**haracteristics of **G**roups via **V**ideo-recorded **A**ctions of **C**rowds — програмна реалізація методу нейромережевого визначення психологічних та соціокультурних характеристик угрупувань за відеофіксацією дій натовпу. Метод базується на детекторі облич **YOLO** (Ultralytics) та трьох незалежно натренованих моделях класифікації **Vision Transformer (ViT)** для визначення віку, статі та емоцій. Має графічний інтерфейс на базі **PyQt6**.
